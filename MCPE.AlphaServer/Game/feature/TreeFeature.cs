...
namespace SpoongePE.Core.Game.feature
{
    public class TreeFeature : Feature
    {
        public override bool place(World world, BedrockRandom rand, int x, int y, int z)
        {
            ...
            for (int i = y; i <= y + 1 + nextInt; i++)
            {
                ...
                for (int i3 = x - i2; i3 <= x + i2 && z2; i3++)
                {
                    for (int i4 = z - i2; i4 <= z + i2 && z2; i4++)
                    {
                        if (i >= 0 && i < 128)
                        {
                            int tileId = world.getBlockIDAt(i3, i, i4);
                            if (tileId != 0 && tileId != Block.leaves.blockID)
                            {
                                z2 = false;
                            }
                        }
            ...
            if (z2)
            {
                int tileId2 = world.getBlockIDAt(x, y - 1, z);
                if ((tileId2 == Block.grass.blockID || tileId2 == Block.dirt.blockID) && y < (128 - nextInt) - 1)
                {
                    ...
                    for (int i5 = (y - 3) + nextInt; i5 <= y + nextInt; i5++)
                    {
                        ...
                        for (int i8 = x - i7; i8 <= x + i7; i8++)
                        {
                            ...
                            for (int i10 = z - i7; i10 <= z + i7; i10++)
                            {
                                ...
                                {
                                    world.placeBlock(i8, i5, i10, (byte)Block.leaves.blockID);
                                }
                            }
                        }
                    }
                    ...
                    {
                        int tileId3 = world.getBlockIDAt(x, y + i12, z);
                        if (tileId3 == 0 || tileId3 == Block.leaves.blockID)
                        {
                            world.placeBlock(x, y + i12, z, (byte)Block.log.blockID);
                        }
                    }
                    return true;
                }
